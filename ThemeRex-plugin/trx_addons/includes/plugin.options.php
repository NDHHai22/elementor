	...
	function trx_addons_load_options() {
		...
		if ( isset( $TRX_ADDONS_STORAGE['options'] ) && is_array( $TRX_ADDONS_STORAGE['options'] ) ) {
			foreach ( $TRX_ADDONS_STORAGE['options'] as $k => $v ) {
				if ( isset( $v['std'] ) ) {
					$val = isset( $_GET[$k] ) 
								? sanitize_text_field( wp_unslash( $_GET[$k] ) )
								: ( isset( $options[$k] )
									? $options[$k]
									: $v['std']
								);
...
if ( ! function_exists( 'trx_addons_get_option' ) ) {
	...
	function trx_addons_get_option( $name, $defa = '', $strict_mode = true ) {
		...
		// If options are not loaded - try to load it
		$present = false;
		if ( ! isset( $TRX_ADDONS_STORAGE['options'] ) ) {
			if ( $options === false ) {
				$options = apply_filters( 'trx_addons_filter_load_options', get_option( 'trx_addons_options' ) );
			}
			$present = isset( $options[ $name ] );
		} else {
			$present = isset( $TRX_ADDONS_STORAGE['options'][ $name ] );
		}
		...
		if ( ! $present && $strict_mode && func_num_args() == 1 && is_user_logged_in() ) {
			// Skip output during AJAX requests to prevent customizer errors
		...
		// Get saved option value (after options are loaded)
		...
		// Get saved option value (before options are loaded)
		} else if ( $options !== false && isset( $options[ $name ] ) ) {
			if ( empty( $part ) ) {
				$rez = $options[ $name ];
			} else if ( isset( $options[ $name ][ $part ] ) ) {
				$rez = $options[ $name ][ $part ];
			}
		}
		return $rez;
	}
	...
	function trx_addons_init_options() {
		...
		$TRX_ADDONS_STORAGE['options'] = apply_filters( 'trx_addons_filter_options', array_merge(
			...
			array(
				...
				'general_section_general' => array(
					"title" => esc_html__('General', 'trx_addons'),
					'icon' => 'trx_addons_icon-cogs',
					"type" => "section"
				),
				'general_info' => array(
			...
			! apply_filters( 'trx_addons_filter_allow_disable_widgets_block_editor', true ) ? array() :
			array(
			...
			array(
				...
				'images_threshold' => array(
					"title" => esc_html__('Big images threshold', 'trx_addons'),
					"desc" => wp_kses_data( __('If the original image width or height is above the threshold, it will be scaled down. The threshold is used as max width and max height. If 0 - turn off image size check.', 'trx_addons') ),
					"std" => 2560,
					"min" => 0,
					"max" => 10000,
					"step" => 10,
				...
				'redirect_404_page' => array(
					"title" => esc_html__('404 Page', 'trx_addons'),
					"desc" => wp_kses_data( __("Select a page to redirect to in case of a 404 error (requested URL not found). If no page is selected - the default page of your theme will be used.", 'trx_addons') ),
					"std" => "",
					"options" => array(),
					"type" => "select"
				),
				...
				'general_section_scroll' => array(
					"title" => esc_html__('Scroll helpers', 'trx_addons'),
					'icon' => 'trx_addons_icon-ioxhost',
					"type" => "section"
				),
				'scroll_info' => array(
					"title" => esc_html__('Scroll helpers', 'trx_addons'),
			...
			array(
				...
				'performance_section_optimize' => array(
					"title" => esc_html__('Optimization', 'trx_addons'),
...
*/
				...
				'performance_section_cache' => array(
					"title" => esc_html__('Cache manager', 'trx_addons'),
					'icon' => 'trx_addons_icon-floppy-1',
					"type" => "section"
				),
				'cache_info' => array(
					"title" => esc_html__('Cache manager', 'trx_addons'),
			...
			array(
				'api_section_google_maps' => array(
					"title" => esc_html__('Google', 'trx_addons'),
					'icon' => 'trx_addons_icon-google',
					"type" => "section"
				),
			...
			array(
				// 'api_section_google_analitics' => array(
			...
			array(
				'api_section_openstreet' => array(
					"title" => esc_html__('OpenStreetMap', 'trx_addons'),
					'icon' => 'trx_addons_icon-map',
					"type" => "section"
				),
			...
			array(
				'api_section_instagram' => array(
					"title" => esc_html__('Instagram', 'trx_addons'),
					'icon' => 'trx_addons_icon-instagram',
					"type" => "section"
				),
			...
			array(
				...
				'theme_specific_section_wrappers' => array(
					"title" => esc_html__('Theme Wrappers', 'trx_addons'),
					"type" => "section"
				),
			),

			! apply_filters( 'trx_addons_filter_allow_theme_columns', true ) ? array() :
			array(
				'columns_info' => array(
...
if ( ! function_exists( 'trx_addons_before_show_options' ) ) {
	...
	function trx_addons_before_show_options( $options, $post_type, $group = '' ) {
		...
		foreach ( $options as $id => $field ) {
			...
			// Fill the options array with pages for 'redirect_404_page' field
			} else if ( $id == 'redirect_404_page' ) {
				$options[$id]['options'] = trx_addons_get_list_pages();
			}
		}
		return $options;
	}
...
if ( ! function_exists( 'trx_addons_init_settings' ) ) {
	...
	function trx_addons_init_settings() {
		...
		$TRX_ADDONS_STORAGE['settings'] = apply_filters( 'trx_addons_init_settings', array(
			...
			'layouts_to_top'            => true,                    // Rename the admin menu item 'Layouts' to 'Theme Layouts' and move it to the top (after Theme Dashboard)
			...
			'redirect_404_way'			=> 'show',					// How to redirect 404 page: redirect|show
																	// 'redirect' - redirect to the permalink of the page selected in the option 'Page to redirect 404'
																	// 'show' - show the page selected in the option 'Page to redirect 404'
		) );
	}
}
...
if ( ! function_exists( 'trx_addons_set_setting' ) ) {
	/**
	 * Set an internal setting value
	 *
	 * @param string $name  Setting name
	 * @param mixed $value  New value
	 */
	function trx_addons_set_setting( $name, $value ) {
		global $TRX_ADDONS_STORAGE;
		if ( isset( $TRX_ADDONS_STORAGE['settings'][ $name ] ) ) {
			$TRX_ADDONS_STORAGE['settings'][ $name ] = $value;
		}
	}
}
