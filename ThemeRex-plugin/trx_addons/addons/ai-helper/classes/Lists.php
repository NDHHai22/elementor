...
//				'' => esc_html__( 'Default model', 'trx_addons' ),
		...
		static function get_list_ai_image_sizes( $api = 'all' ) {
			...
			return apply_filters( 'trx_addons_filter_ai_helper_list_ai_image_sizes', array_merge(
					...
					in_array( $api, array( 'all', 'openai', 'sd', 'stable-diffusion' ) ) && ( $openai_enable || $sd_enable )
						? array(
					...
					in_array( $api, array( 'all', 'sd', 'stable-diffusion' ) ) && $sd_enable
						? array(
					...
					in_array( $api, array( 'all', 'stability-ai' ) ) && $stability_enable
						...
						: array(),
					// Stable Diffusion and Stability AI only
					in_array( $api, array( 'all', 'sd', 'stable-diffusion', 'stability-ai' ) ) && ( $sd_enable || $stability_enable )
						? array(
		...
		static function get_list_ai_image_apis() {
			return apply_filters( 'trx_addons_filter_ai_helper_list_ai_image_apis', array(
				'openai' => esc_html__( 'Open AI', 'trx_addons' ),
				'stable-diffusion' => esc_html__( 'Stable Diffusion', 'trx_addons' ),
			...
			foreach( $api_order as $api => $enable ) {
				...
				if ( $api == 'stable-diffusion' && (int)$enable > 0 ) {
					...
					if ( is_array( $sd_models ) && count( $sd_models ) > 0 ) {
						if ( $groups ) {
							$models[ 'stable-diffusion/-' ] = '\\-' . __( 'Stable Diffusion models', 'trx_addons' );
						}
		...
		/**
		 * Return a list of image models for Google Ai
		 * 
		 * @access public
		 * @static
		 * 
		 * @return array  	  The list of image models for Google Ai
		 */
