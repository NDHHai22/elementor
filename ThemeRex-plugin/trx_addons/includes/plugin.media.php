...
if ( ! function_exists( 'trx_addons_set_images_threshold' ) ) {
	// Hook this after WordPress fully loads to ensure options are available
	add_action( 'wp_loaded', function() {
		add_filter( 'big_image_size_threshold', 'trx_addons_set_images_threshold', 10, 1 );
	} );
	/**
	 * Set a new threshold to check image size when upload them to the media library. 2560 by default.
	 * 
	 * @hooked big_image_size_threshold
	 * 
	 * @param  int $threshold    Default threshold. 2560 by default.
	 * 
	 * @return int               New threshold.
	 */
	function trx_addons_set_images_threshold( $threshold = 2560 ) {
		$t = (int) trx_addons_get_option( 'images_threshold', 2560, false );
		return $t === 0 ? false : max( 1, min( 10000, $t ) );
	}
}
...
if ( ! function_exists( 'trx_addons_set_images_quality' ) ) {
	...
	function trx_addons_set_images_quality( $defa = 90, $mime = '' ) {
		$q = (int) trx_addons_get_option( 'images_quality', 60, false );
		if ( $q == 0 ) $q = 90;
			...
			if ( $args['mode'] == 'gallery' ) {						// Get images from first gallery in the current post
				...
				while ( $query->have_posts() ) { $query->the_post();
					...
					$images[] = apply_filters('trx_addons_filter_slider_content', array(
						...
						'date' => apply_filters('trx_addons_filter_slider_query_date', false ) ? apply_filters( 'trx_addons_filter_get_post_date', get_the_date() ) : '',
						'link' => get_permalink()
						),
						$args);
					if ( $num >= $args['count'] ) {
