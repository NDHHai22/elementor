...
class Preferences {
	...
	protected function add_personal_options_settings( \WP_User $user ) {
		...
		// Don't show checkbox for administrator users.
	...
	protected function update_personal_options_settings( $user_id ) {
		...
		$requested_has_capability = '1' === $requested_angie_value;

		// Update capability based on the requested value.
		if ( $requested_has_capability && ! $current_has_capability ) {
			// Grant capability.
			Angie_Capability_Manager::grant_capability_to_user( $user_id );
		} elseif ( ! $requested_has_capability && $current_has_capability ) {
			// Revoke capability.
